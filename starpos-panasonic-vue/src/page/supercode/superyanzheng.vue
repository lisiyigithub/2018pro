  ...
  <div class="newyanzheng">
    ...
    <div class="main">
      <yd-cell-group>
        ...
        <yd-cell-item arrow class="sfz">
          ...
          <yd-datetime :start-date="current" end-date="2099-12-31" v-model="crp_exp_dt_tmp" type="date" slot="right"
        ...
        <yd-cell-item>
          ...
          <yd-input ref="stl_oac" slot="right" v-model="stl_oac" regex="/^\d{16,21}$/" placeholder="只支持储蓄卡" required
            :show-success-icon="false" :show-error-icon="false" :show-clear-icon="false"></yd-input>
        </yd-cell-item>
        <yd-cell-item>
          <span slot="left">账户户名
            <!-- <i>*</i> -->
          </span>
          <yd-input ref="bnk_acnm" slot="right" v-model="bnk_acnm" regex="^(?![0-9]*$)[\u4e00-\u9fa5a-zA-Z0-9_]{2,10}$"
            placeholder="请输入账户户名" required :show-success-icon="false" :show-error-icon="false" :show-clear-icon="false"></yd-input>
        </yd-cell-item>
        ...
        <yd-cell-item>
          ...
          <yd-input ref="stoe_nm" slot="right" v-model="stoe_nm" regex="^(?![0-9]*$)[\u4e00-\u9fa5a-zA-Z0-9_]{8,20}$"
            placeholder="请输入商户名称" required :show-success-icon="false" :show-error-icon="false" :show-clear-icon="false"></yd-input>
        </yd-cell-item>
        <yd-cell-item>
          <span slot="left">联系人
            <!-- <i>*</i> -->
          </span>
          <yd-input ref="stoe_cnt_nm" slot="right" v-model="stoe_cnt_nm" regex="^(?![0-9]*$)[\u4e00-\u9fa5a-zA-Z0-9_]{2,10}$"
            placeholder="请输入联系人" required :show-success-icon="false" :show-error-icon="false" :show-clear-icon="false"></yd-input>
        </yd-cell-item>
        <yd-cell-item>
          <span slot="left">手机号码
            <!-- <i>*</i> -->
          ...
          <yd-input ref="stoe_cnt_tel" slot="right" v-model="stoe_cnt_tel" regex="/^(0|86|17951)?1([358][0-9]|4[579]|66|7[0135678]|9[89])[0-9]{8}$/"
            placeholder="请输入手机号码" readonly="false" required :show-success-icon="false" :show-error-icon="false" :show-clear-icon="false"></yd-input>
        </yd-cell-item>
    ...
    <yd-popup class="verification-code" v-model="show" position="center" width="75%" :close-on-masker='false'>
      <div style="background-color:#f8f8f8;border-radius: 12px;padding:10px;">
        ...
        <img class="close" src="../../assets/cha.png" @click="show = false">
        <p>正在发送验证码{{stoe_cnt_tel}}</p>
        <p> 即将为您签署电子合同协议</p>
        <p>请输入短信验证码确认</p>
        <yd-cell-item>
...
<script>
  import uploader from "../../components/uploader4";
  import uploaderTwo from "../../components/uploader5";
  ...
  export default {
    data() {
      return {
        ...
        foy_img: "", //	场景照 2
        choc_img: "", //	收银台照 3
        merc_bankcode_img: "1", //	结算人身份证 4
        mer_con1_img: "", //ME50其他1 手持身份证
        mer_con2_img: "", //ME50其他2 人门合照
        hold_img: "1", //	结算人手持身份证 5，ME50身份证反面5
        met_img: "1", //	银行卡照 6
    ...
    created() {
      ...
      if (util.sesstionData("PublicParams") != false) {
        ...
        this.stoe_cnt_tel = paramsObj.mbl_no
      }
      //判断是否为修改状态  是就获取数据
    ...
    methods: {
      ...
      checkMerc() {
        ...
        if (_this.isUpdate) {
          ...
          data2 = {
            ...
            terminl_num: "1", //终端数量
            check_typ: "0",
            invite_no: this.invitation_code, //邀请码
            //照片
            merc_bankcode_img: _this.merc_bankcode_img, //	结算人身份证 4
            hold_img: _this.hold_img, //	结算人手持身份证 5
            met_img: _this.met_img //	银行卡照 6
        ...
        axios({
            method: "post",
            url: `${api.api}checkUpdateMerc.json`,
            data: data2
          })
          .then(res => {
            _this.$dialog.loading.close();
      ...
      mercRegister() {
        ...
        let data = {
          ...
          terminl_num: "1", //终端数量
          invite_no: _this.invitation_code, //邀请码
          //照片
          merc_bankcode_img: _this.merc_bankcode_img, //	结算人身份证 4
          hold_img: _this.hold_img, //	结算人手持身份证 5
          met_img: _this.met_img, //	银行卡照 6
        ...
        console.log(data);
        var url = `${api.api}publicMercRegister.json`;
        if (_this.isUpdate) {
          url = `${api.api}UpdateMerc.json`;
      ...
      check2(value) {
        ...
        switch (_this.step) {
          case 1:
            //console.log(_this.myItems1);
            _this.opn_bnk_city = value;
            break;
          case 2:
            for (let i = 0; i < _this.myItems4.length; i++) {
              if (value == _this.myItems4[i].label) {
                _this.wc_lbnk_no = _this.myItems4[i].wc_lbnk_no;
                _this.lev3 = value;
              }
            }
            break;
