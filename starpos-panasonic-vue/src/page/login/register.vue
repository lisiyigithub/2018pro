    ...
    <div class="main">
      <yd-cell-group>
        ...
        <yd-cell-item>
          ...
          <yd-input  ref="input2" type="number" slot="right" v-model='mobile_no' placeholder="请输入手机号" regex="/^(0|86|17951)?(13[0-9]|15[012356789]|17[678]|18[0-9]|14[57])[0-9]{8}$/"></yd-input>
        </yd-cell-item>
        <yd-cell-item>
      ...
      <p class="p1">
        <router-link :to="{ name: 'login',params: { 'openid': openid }}">已有账号，立即登陆</router-link>
      </p>
    </div>
    <footer></footer>
  </div>
  ...
  methods: {
    ...
    queryCode() {
      ...
      if (this.$refs.input2.valid && this.mobile_no == "") {
        this.$dialog.toast({ mes: "请输入正确的手机号码", timeout: 2000 });
        return false;
      } else {
        let data = {
    ...
    check() {
      if (this.$refs.input1.valid && this.user_name != "") {
      } else {
        this.$dialog.toast({ mes: "请输入正确的用户名", timeout: 2000 });
        return false;
      }
      if (this.$refs.input2.valid && this.mobile_no != "") {
      } else {
      ...
      if (this.verify_code == "") {
        this.$dialog.toast({ mes: "验证码不能为空", timeout: 2000 });
        return false;
      }
      let re = /^(?![\d]+$)(?![a-zA-Z]+$)(?![!#$%^&*]+$)[\da-zA-Z!#$%^&*]{6,20}$/;
      if (re.test(this.usr_pwd) && re.test(this.usr_pwd2)) {
      } else {
        this.$dialog.toast({ mes: "密码格式不正确", timeout: 2000 });
        return false;
      }
      if (this.usr_pwd != this.usr_pwd2) {
        this.$dialog.toast({ mes: "两次密码不一致", timeout: 2000 });
        return false;
      }
    },
    submit() {
